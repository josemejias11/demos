...
test.describe('jbs.dev Accessibility', () => {
  ...
  test('Interactive elements are keyboard accessible', async ({ page }) => {
    ...
    for (const element of buttons.slice(0, 5)) { // Test first 5 to keep test fast
      ...
      // Check if element is visible and not disabled before testing focus
      const isVisible = await element.isVisible();
      const isDisabled = await element.evaluate(el => (el as HTMLButtonElement).disabled);
      ...
      try {
        ...
        // Skip elements that can't be focused due to page structure
        continue;
      }
    }
  });
